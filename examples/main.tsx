...
const schema = yup.object().shape<Form>({
  first_name: yup
    .string()
    .test("expensive", "expensive validation", async (value) => {
    ...
    .shape({
      ...
      number: yup
        ...
        .test(
          ...
          (value: Form["phone"]["number"]) => String(Number(value)) !== value
        )
        .required(),
    })
    .required(),
  orders: yup
    ...
    .test(
      "quantity",
      "you can't order more than 100 items",
      (orders: Form["orders"]) => {
        console.log("test 1");
    ...
    .test(
      "no duplicates",
      "you can't order twice the same product",
      (orders: Form["orders"]) => {
...
function Orders({ orders }: Orders) {
  ...
  return (
    <fieldset>
      ...
      <button
        type="button"
        onClick={() => orders.array.push({ product: "", quantity: 0 })}
      >
        Add order
      </button>
...
type OrderProps = {
  ...
  orderLense: form.Lense<any, Form["orders"][number]>;
};
...
function FieldState<VALUE>({
  field,
}: FieldStateProps<VALUE>): React.ReactElement<FieldStateProps<VALUE>> {
  return (
    <div>
      <div>id: {field.meta.id}</div>
      <div>
        error: <pre>{JSON.stringify(field.meta.errors)}</pre>
      </div>
      <div>validity: {field.valid() ? "true" : "false"}</div>
      <div>
        value: <pre>{JSON.stringify(field.value(), null, 2)}</pre>
      </div>
